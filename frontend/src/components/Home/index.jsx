...
const Home = () => {
  ...
  useEffect(() => {
    Object.keys(contentRefs).forEach((key) => {
      ...
      if (contentRef) {
        contentRef.style.maxHeight = activeSections[key] ? `${contentRef.scrollHeight}px` : '0px';
      }
    });
  ...
  return (
    <div className="home">
      ...
      <div className="welcome-page">
        ...
        <button className={`accordion-button ${activeSections.event ? 'active' : ''}`}
                onClick={() => toggleAccordion('event')}>
          <div>
            <div className="faq-section">
              <h2 className="h2-home">El evento</h2>
              <FontAwesomeIcon icon={faPlus} className={activeSections.event ? `question-icon rotate` : `question-icon`} />            </div>
            <div className={activeSections.event ? `accordion-content answer-divider` : `accordion-content`}
                 ref={contentRefs.event}><h3 className="h3-home">Cuándo</h3>
              <p>La ceremonia será el 5 de Octubre de 2024 a las 18:00. Después pasaremos al cocktail, la cena y el baile.</p>
        ...
        <button className={`accordion-button ${activeSections.information ? 'active' : ''}`}
                onClick={() => toggleAccordion('information')}>
          <div>
            <div className="faq-section">
              <h2 className="h2-home">Más información</h2>
              <FontAwesomeIcon icon={faPlus} className={activeSections.information ? `question-icon rotate` : `question-icon`} />            </div>
            <div className={activeSections.information ? `accordion-content answer-divider` : `accordion-content`}
                 ref={contentRefs.information}>
              <h3 className="h3-home">Alojamiento</h3>
              <p>En la propia finca hay disponible alojamiento (bungalows). Hay más información en <Link to="https://www.complejolaciguena.com/bungalows-madrid/">su web</Link></p>
              ...
              <p>Los precios los tenéis disponibles <a href={preciosPDF} target="_blank" rel="noopener noreferrer">aquí</a>.</p>              <h3 className="h3-home">Planes de domingo</h3>
